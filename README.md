...
Для начала скрейпинга можно просто запустить `main.ipynb`, предварительно создав виртуальное окружение и установив библиотеки из `requirements.txt`. Рекомендуется выставить переменные `GET_FROM`, `GET_TO` и `DOC_LIMIT` для ограничения скрейпинга статей.

> Примечание: сейчас скрейпинг происходит без параллелизма - последовательно находятся источники статей, происходит их скачивание, парсинг и сохранение. В будущем хочется переписать это все на асинхронный рантайм с асинхронными очередями. 